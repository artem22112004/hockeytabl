...
function GameCard({ game }: { game: any }) {
  ...
  return (
    <Link
      href={`/match-center/${game.gameId}`}
      className={`flex flex-col gap-3 rounded-xl border bg-surface p-4 text-left transition-all hover:border-[#00D1FF]/40 hover:bg-hover hover:shadow-[0_4px_16px_rgba(0,209,255,0.08)] ${
        isLive ? 'border-red-700/50' : 'border-border'
      }`}
    >
      <div className="flex items-center gap-2">
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <p className="font-mono text-[11px] text-muted">{game.awayTeam.abbrev}</p>
          </div>
        </div>
        ...
        <div className="flex min-w-0 flex-1 items-center justify-end gap-2">
          <div className="min-w-0 text-right">
            ...
            <p className="font-mono text-[11px] text-muted">{game.homeTeam.abbrev}</p>
          </div>
          <Image
...
function LoadingSpinner() {
  return (
    <div className="flex items-center justify-center py-24">
      <div className="h-10 w-10 animate-spin rounded-full border-4 border-border border-t-[#00D1FF]" />
    </div>
  );
}
